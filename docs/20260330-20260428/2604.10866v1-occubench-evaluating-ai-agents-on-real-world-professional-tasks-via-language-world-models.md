...
tags: ["query:malm"]
score: 8.0
evidence: 专业任务的多智能体合成流水线
tldr: OccuBench是一个评估AI智能体在100个真实专业任务场景（涵盖10个行业、65个领域）中表现的新基准。它利用语言世界模型（LWM）模拟特定领域的环境，并通过多智能体合成流水线生成具有可解性和多样性的评估实例。该基准从任务完成度和环境鲁棒性两个维度评估了15个前沿模型，揭示了模型在不同行业的能力差异及对隐式故障的处理挑战。
selection_source: fresh_fetch
motivation: 现有的AI智能体基准测试受限于公开环境，难以评估智能体在众多专业职业领域（如核反应堆监控、海关处理等）的实际表现。
method: 提出OccuBench基准，利用语言世界模型（LWM）模拟领域环境，并通过多智能体合成流水线自动生成包含故障注入（显式错误、隐式数据降级）的评估实例。
result: 评估发现没有单一模型能统治所有行业，隐式故障比显式错误更难处理，且增加推理投入能显著提升模型性能。
conclusion: OccuBench为AI智能体在跨行业专业任务上的表现提供了首次系统性评估，强调了环境模拟质量对评估可靠性的重要性。
---

## 摘要
AI 智能体被期望在数百个职业领域（从急诊室分诊到核反应堆安全监测，再到海关进口处理）执行专业工作，然而现有的基准测试只能在少数存在公共环境的领域评估智能体。我们推出了 OccuBench，这是一个涵盖 10 个行业类别和 65 个专业领域的 100 个现实世界专业任务场景的基准测试，由语言世界模型（LWMs）通过 LLM 驱动的工具响应生成来模拟特定领域环境。我们的多智能体合成流水线可自动生成具有可解性保证、校准难度和基于文档多样性的评估实例。OccuBench 从两个互补维度评估智能体：跨专业领域的任务完成度，以及在受控故障注入（显式错误、隐式数据降级和混合故障）下的环境鲁棒性。我们评估了 8 个模型家族中的 15 个前沿模型，发现：(1) 没有单一模型能主导所有行业，每个模型都有独特的职业能力画像；(2) 隐式故障（数据截断、字段缺失）比显式错误（超时、500 错误）和混合故障更难，因为它们缺乏明显的错误信号，需要智能体自主检测数据降级；(3) 更大的模型、更新的代际和更高的推理投入能持续提升性能，GPT-5.2 在从最小到最大推理投入下提升了 27.5 分；(4) 强大的智能体并不一定是强大的环境模拟器，模拟器质量对基于 LWM 的评估可靠性至关重要。OccuBench 提供了首个针对 AI 智能体在专业职业任务上的系统性跨行业评估。
