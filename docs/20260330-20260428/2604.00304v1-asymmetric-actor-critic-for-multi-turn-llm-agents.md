...
tags: ["query:malm"]
score: 9.0
evidence: 多轮大语言模型智能体的不对称演员-评论家框架
tldr: 针对多轮LLM智能体在单次交互中可靠性不足的问题，本文提出一种非对称Actor-Critic框架。该框架利用强大的闭源模型作为Actor，配合轻量级开源模型作为Critic进行实时监督与干预。通过利用生成与验证之间的不对称性，在不修改Actor的前提下，通过微调Critic显著提升任务成功率。实验证明该方法在τ-bench等基准上优于单智能体基线，且小模型Critic表现优异。
selection_source: fresh_fetch
motivation: 现有的多轮对话智能体在无法重试的场景中可靠性较差，且难以在利用闭源模型的同时实现有效的实时监督。
method: 提出非对称Actor-Critic框架，由闭源大模型担任Actor，微调后的轻量级开源模型担任Critic，在同一交互轨迹内进行实时监控与干预。
result: 在τ-bench和UserBench上的实验显示，该方法显著提升了任务成功率，且微调后的小模型Critic性能可媲美或超越大型闭源模型。
conclusion: 非对称Actor-Critic框架证明了通过轻量级模型对强大Actor进行实时监督是提升多轮对话智能体可靠性的有效且高效的途径。
---

## 摘要
大语言模型（LLMs）展现出强大的推理和对话能力，但在多轮交互中确保行为的可靠性仍然具有挑战性。在许多现实应用中，智能体必须在无法重试的单次尝试场景下取得成功。现有方法要么依赖于反思或事后评估（这需要额外的尝试），要么假设模型是完全可训练的，从而无法利用闭源 LLMs。我们提出了一种用于可靠对话智能体的非对称 Actor-Critic 框架。该框架由一个强大的闭源 LLM 担任 Actor，而一个较小的开源 Critic 提供运行时监督，监控 Actor 的动作并在同一交互轨迹内进行干预。与基于训练的 Actor-Critic 方法不同，我们的框架监督的是在开放式对话环境中运行的固定 Actor。该设计利用了生成与验证之间的不对称性：虽然高质量的生成需要大模型，但有效的监督通常可以通过较小的模型来实现。我们还引入了一个数据生成流水线，在不修改 Actor 的情况下产生用于 Critic 微调的监督信号。在 $τ$-bench 和 UserBench 上的实验表明，与强大的单智能体基准相比，我们的方法显著提高了可靠性和任务成功率。此外，轻量级开源 Critic 在担任 Critic 角色时可以媲美甚至超越更大的闭源模型，且 Critic 微调在几种最先进方法的基础上带来了额外的增益。
