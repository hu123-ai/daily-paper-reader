...
title_zh: 面向可靠专利权利要求验证的自适应低成本评估
authors: "Yongmin Yoo, Qiongkai Xu, Longbing Cao"
date: 2026-04-05
pdf: "https://arxiv.org/pdf/2604.04295v1"
tags: ["query:q2"]
score: 9.0
evidence: 大语言模型用于专利权利要求验证和法律依赖分析
tldr: "针对专利权利要求验证中轻量级模型精度不足与大语言模型（LLM）成本过高的问题，本文提出ACE框架。该框架通过预测熵识别高不确定性样本并将其路由至专家级LLM，结合基于法律标准的专利思维链（CoPT）进行深度验证。实验表明，ACE在保持94.95%高F1分数的同时，比独立LLM部署降低了78%的成本，并贡献了包含4万条标注数据的ACE-40k基准数据集。"
selection_source: fresh_fetch
motivation: 专利验证要求极高的准确性，但现有方法在处理复杂的法律依赖关系时面临性能与成本的权衡难题。
method: 提出一种混合框架，利用预测熵动态路由高难度样本至采用专利思维链协议的专家级大模型进行处理。
result: "ACE框架实现了94.95%的F1分数，且相比于直接使用大模型，其运行成本大幅降低了78%。"
conclusion: 该研究通过自适应路由机制有效平衡了专利验证的可靠性与经济性，为法律领域的自动化评估提供了新思路。
---

## 摘要
专利权利要求的自动验证要求零缺陷容忍，因为即使是单一的结构性缺陷也可能导致权利要求在法律上失效。现有的评估范式面临着刚性与资源的困境：轻量级编码器难以处理细微的法律依赖关系，而通过大语言模型（LLM）进行的详尽验证则成本极高。为了弥补这一差距，我们提出了 ACE（自适应低成本评估），这是一个混合框架，利用预测熵仅将高不确定性的权利要求路由至专家级 LLM。随后，专家模型执行基于美国法典第 35 卷（35 U.S.C.）法定标准的专利思维链（CoPT）协议。这一设计使 ACE 能够更有效地处理长程法律依赖关系，同时保持效率。ACE 在评估的方法中取得了 94.95% 的最佳 F1 分数，同时与独立部署 LLM 相比，运营成本降低了 78%。我们还构建了 ACE-40k，这是一个包含 40,000 条权利要求的基准测试集，具有基于 MPEP（专利审查程序指南）的错误标注，以促进进一步研究。
