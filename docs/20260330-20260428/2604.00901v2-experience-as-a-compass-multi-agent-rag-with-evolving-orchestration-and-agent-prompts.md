...
tags: ["query:malm"]
score: 9.0
evidence: 具有演化编排功能的多智能体RAG分层框架
tldr: 针对多智能体检索增强生成（RAG）中静态编排和固定行为导致的性能瓶颈，本文提出HERA框架。该框架通过分层演化机制，在全局层面利用奖励引导优化智能体拓扑结构，在局部层面通过角色感知提示词演化精炼智能体行为。实验表明，HERA在多个知识密集型任务上显著提升了性能和效率，展现了出色的自组织与推理能力。
selection_source: fresh_fetch
motivation: 现有的多智能体RAG系统依赖静态编排和固定提示词，难以应对复杂多变的多跳查询任务。
method: 提出HERA框架，结合全局层面的查询特定拓扑优化与局部层面的角色感知提示词演化，实现编排与行为的协同进化。
result: "在六个知识密集型基准测试中，HERA相比现有基准平均提升了38.69%，并表现出极高的Token利用效率。"
conclusion: 通过经验积累和双层演化，多智能体系统能够自发形成高效的协作网络，显著增强复杂推理任务的鲁棒性。
---
