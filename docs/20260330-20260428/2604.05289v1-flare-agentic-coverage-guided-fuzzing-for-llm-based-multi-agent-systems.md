...
title_zh: FLARE：针对基于大语言模型的多智能体系统的智能体覆盖引导模糊测试
...
tags: ["query:malm"]
score: 9.0
evidence: 多智能体大语言模型系统的测试框架
tldr: 针对多智能体大模型系统（MAS）中因非确定性和复杂交互导致的失效问题，本文提出了FLARE测试框架。FLARE通过从源代码中提取规范和行为空间，构建测试预言机，并采用覆盖率引导的模糊测试技术来暴露系统缺陷。实验表明，FLARE在16个开源应用中实现了极高的代理间和代理内覆盖率，并发现了56个此前未知的MAS特有故障，显著提升了系统的可靠性。
selection_source: fresh_fetch
motivation: 传统软件测试方法难以有效检测多智能体系统因非确定性交互、缺乏规范及语义复杂性而产生的各类故障。
method: FLARE从MAS源代码中提取行为规范，构建测试预言机，并利用覆盖率引导的模糊测试来系统性地暴露潜在失效。
result: "在16个开源应用上的评估显示，FLARE实现了96.9%的代理间覆盖率，并成功发现了56个此前未知的MAS特有故障。"
conclusion: FLARE证明了结合规范提取与覆盖率引导的模糊测试是保障多智能体大模型系统可靠性的有效途径。
---

## 摘要
多智能体大语言模型系统（MAS）已被用于通过将任务分解为子任务来自动化复杂的人类工作流。然而，由于大语言模型智能体的非确定性行为以及智能体之间复杂的交互，MAS 应用经常遇到故障，包括死循环和工具调用失败。由于缺乏大语言模型智能体规范、MAS 庞大的行为空间以及基于语义的正确性判定，传统的软件测试技术在检测此类故障方面效果不佳。本文提出了 FLARE，一种专为 MAS 量身定制的新型测试框架。FLARE 以 MAS 的源代码作为输入，并从智能体定义中提取规范和行为空间。基于这些规范，FLARE 构建测试断言（test oracles）并进行覆盖引导的模糊测试以暴露故障。随后，它分析执行日志以判断每个测试是否通过，并生成故障报告。我们对 16 个不同的开源应用进行的评估表明，FLARE 实现了 96.9% 的智能体间覆盖率和 91.1% 的智能体内覆盖率，分别比基准方法高出 9.5% 和 1.0%。FLARE 还发现了 56 个 MAS 特有的、此前未知的故障。
