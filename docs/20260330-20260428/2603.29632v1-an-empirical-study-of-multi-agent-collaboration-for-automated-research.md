...
tldr: 本研究针对自动化科研中的多智能体协作框架进行了系统性实证研究。通过在受控的机器学习优化测试平台上对比单智能体与两种多智能体架构（子智能体并行模式与专家团队协作模式），揭示了操作稳定性和理论深度之间的权衡。研究发现，子智能体模式适合限时广度搜索，而专家团队模式在充足算力下更擅长复杂架构重构，为未来自动化科研系统的动态架构设计提供了指导。
selection_source: fresh_fetch
motivation: 旨在探索在自动化科研任务中，如何通过最优的多智能体协作框架来克服单模型系统的认知瓶颈。
method: 在配备Git隔离和全局记忆的受控测试平台上，对比分析了单智能体、子智能体并行架构和专家团队协作架构在机器学习优化任务中的表现。
result: 实验表明子智能体模式在限时任务中具备高韧性和高吞吐量，而专家团队模式在充足预算下能实现更深层的理论对齐和复杂重构。
conclusion: 未来自动化科研系统应采用动态路由架构，根据任务复杂度和实时需求灵活调整其多智能体协作结构。
---

## 摘要
随着人工智能智能体（AI agents）的演进，研究界正迅速从单一大型语言模型（LLMs）转向多智能体系统（MAS），以克服自动化研究中的认知瓶颈。然而，针对这些自主智能体的最优多智能体协调框架在很大程度上仍未得到探索。本文通过一项系统的实证研究，调查了不同多智能体结构在自动化机器学习优化中的比较效能。利用一个配备了 Git 工作树隔离和显式全局内存的严格受控、基于执行的测试平台，我们将单智能体基准与两种多智能体范式进行了对比：子智能体架构（事后整合的并行探索）和智能体团队架构（执行前交接的专家协作）。通过在严格固定的计算时间预算下评估这些系统，我们的研究结果揭示了运行稳定性与理论深思（theoretical deliberation）之间的一种根本性权衡。子智能体模式表现为一个高弹性、高吞吐量的搜索引擎，最适合在严格的时间限制下进行广泛且浅层的优化。相反，智能体团队拓扑结构由于多作者代码生成而表现出更高的运行脆弱性，但在计算预算充足的情况下，能够实现复杂架构重构所需的深度理论对齐。这些实证见解为设计未来的自动化研究系统提供了可操作的指导方针，提倡采用能够根据实时任务复杂度调整其协作结构的动态路由架构。
