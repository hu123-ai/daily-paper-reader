...
tags: ["query:malm"]
score: 9.0
evidence: 多智能体协作中角色一致性的架构
tldr: 针对大模型多智能体协作中常见的角色偏离和越权问题，本文提出了一种量化角色清晰度的方法。该方法通过构建角色分配矩阵来衡量智能体行为轨迹与角色描述之间的语义相似度，并将其作为正则化项引入轻量化微调中。实验证明，该方法在ChatDev系统上显著降低了角色越权率，提升了角色一致性及最终的任务成功率。
selection_source: fresh_fetch
motivation: 旨在解决多智能体系统中智能体因无法严格遵守角色定义而导致的行为混淆和协作效率低下的问题。
method: 通过计算行为轨迹与角色描述的语义相似度构建角色清晰度矩阵，并将其作为正则化约束进行轻量化微调。
result: "实验显示角色越权率大幅下降（如Llama从43.4%降至0.2%），同时角色清晰度评分和任务成功率均有显著提升。"
conclusion: 量化角色清晰度并结合正则化微调是提升多智能体系统角色一致性和协作性能的有效途径。
---

## 摘要
在大语言模型（LLM）驱动的多智能体系统中，违背角色规范（即未能遵守所分配角色的定义职责和约束，可能导致一个智能体的行为表现得像另一个智能体）是一种主要的失效模式。为解决这一问题，本文提出了一种定量角色清晰度方法，旨在提升角色一致性。首先，我们构建了一个角色分配矩阵 $S(φ)=[s_{ij}(φ)]$，其中 $s_{ij}(φ)$ 表示第 $i$ 个智能体的行为轨迹与第 $j$ 个智能体的角色描述之间的语义相似度。随后，我们将角色清晰度矩阵 $M(φ)$ 定义为 $\text{softmax}(S(φ))-I$，其中 $\text{softmax}(S(φ))$ 是 $S(φ)$ 的逐行 Softmax 结果，$I$ 是单位矩阵。$M(φ)$ 的 Frobenius 范数衡量了智能体角色描述与其行为轨迹之间的对齐程度。此外，我们在轻量化微调过程中将角色清晰度矩阵作为正则化项，以提升角色一致性，从而提高端到端任务性能。在 ChatDev 多智能体系统上的实验表明，我们的方法显著提升了角色一致性和任务性能：在使用 Qwen 和 Llama 时，角色越权率分别从 $46.4\%$ 降至 $8.4\%$ 以及从 $43.4\%$ 降至 $0.2\%$；角色清晰度得分分别从 $0.5328$ 提升至 $0.9097$ 以及从 $0.5007$ 提升至 $0.8530$；任务成功率则分别从 $0.6769$ 提升至 $0.6909$ 以及从 $0.6174$ 提升至 $0.6763$。
