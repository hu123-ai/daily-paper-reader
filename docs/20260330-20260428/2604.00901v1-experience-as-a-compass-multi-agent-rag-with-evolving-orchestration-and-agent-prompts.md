...
title_zh: 经验如指南：具有演进式编排与智能体提示词的多智能体检索增强生成
authors: "Sha Li, Naren Ramakrishnan"
date: 2026-04-01
pdf: "https://arxiv.org/pdf/2604.00901v1"
tags: ["query:malm"]
score: 9.0
evidence: 具有演化编排和智能体提示的多智能体检索增强生成
tldr: 针对多智能体检索增强生成（RAG）在处理复杂多跳任务时表现僵化的问题，本文提出HERA框架。该框架通过分层演化机制，在全局层面利用奖励引导优化查询特定的智能体拓扑结构，在局部层面通过角色感知提示词演化精炼智能体行为。实验表明，HERA在六个知识密集型基准测试中性能提升显著，展现了高效的协调能力和鲁棒的推理性能。
selection_source: fresh_fetch
motivation: 现有的多智能体RAG系统依赖静态编排和固定行为，难以适应多样化且复杂的多跳查询任务。
method: 提出HERA框架，结合全局层面的奖励引导拓扑优化与局部层面的角色感知提示词演化，实现编排与行为的协同进化。
result: "在六个基准测试中，HERA相比现有基准平均性能提升了38.69%，并表现出极高的Token利用效率。"
conclusion: 通过经验积累和分层演化，多智能体系统能够自发形成高效的协作网络，显著增强复杂推理任务的处理能力。
---

## 摘要
多智能体检索增强生成（RAG）通过让每个智能体承担特定角色，支持需要多步骤、多来源或复杂推理的难题。然而，现有方法依赖于静态的智能体行为和固定的编排策略，导致在多样化的多跳任务中表现脆弱。我们识别出两个关键局限：缺乏持续自适应的编排机制，以及单个智能体缺乏行为层面的学习。为此，我们提出了 HERA，这是一个分层框架，能够协同演进多智能体编排和特定角色的智能体提示词。在全局层面，HERA 通过奖励引导的采样和经验积累来优化针对特定查询的智能体拓扑结构。在局部层面，“角色感知提示词演进”通过信用分配以及沿操作和行为原则的双轴自适应来优化智能体行为，从而实现有针对性的、基于角色的改进。在六个知识密集型基准测试中，HERA 相比于近期的基准模型实现了平均 38.69% 的性能提升，同时保持了稳健的泛化能力和 Token 效率。拓扑分析揭示了涌现的自组织现象，其中稀疏探索产生了紧凑且高实用性的多智能体网络，展示了高效的协作和稳健的推理能力。
