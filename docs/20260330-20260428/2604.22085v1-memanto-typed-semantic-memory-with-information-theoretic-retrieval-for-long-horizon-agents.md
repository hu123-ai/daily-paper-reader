...
motivation: 现有基于知识图谱或向量的内存系统在数据摄取和检索时存在高计算开销、高延迟及复杂的模式维护问题。
method: 引入了包含13个预定义类别的类型化语义内存架构，结合冲突解决与时间版本化，并采用无需索引的信息论搜索引擎实现快速检索。
result: "在LongMemEval和LoCoMo基准测试中分别取得89.8%和87.1%的准确率，显著优于现有的混合图和向量化系统。"
conclusion: Memanto证明了通过简化架构和高效检索算法，可以在降低系统复杂度的同时实现高性能的长程智能体内存管理。
---
