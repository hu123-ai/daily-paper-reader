...
tags: ["query:malm"]
score: 10.0
evidence: LLM系统智能体通信协议分析
tldr: 本研究针对大语言模型智能体系统的通信协议，提出了由通信、语法和语义三层构成的分析框架。通过对18种代表性协议的系统分析，揭示了当前协议在传输和结构化交互方面日趋成熟，但在语义层面的澄清、上下文对齐和验证机制上存在显著不足。文章识别了现有生态中的技术债，并为不同场景下的协议选择提供了指导，旨在推动智能体间实现超越简单消息传递的深度语义协作。
selection_source: fresh_fetch
motivation: 随着LLM智能体系统在异构环境下的协作需求日益增长，现有的通信协议缺乏系统化的评估标准和对语义一致性的支持。
method: 提出了一种受人类启发的三层通信框架（通信、语法、语义），并对18种主流智能体通信协议进行了多维度的对比分析。
result: 分析显示当前协议在传输和模式定义上已较为成熟，但在语义层面的协调机制上存在明显缺失，导致语义负担被转嫁至提示词和应用逻辑中。
conclusion: 未来的研究应超越简单的消息传递，致力于构建具备互操作性、安全且能实现共享理解的语义鲁棒型智能体生态系统。
---

## 摘要
智能体通信协议正成为大语言模型（LLM）系统的关键基础设施，这些系统必须使用工具、与其他智能体协作并在异构环境中运行。本研究通过将智能体通信组织为三个层级（通信层、句法层和语义层），为这一新兴领域提供了一个受人类启发的视角。在该框架下，我们系统地分析了 18 种具有代表性的协议，并比较了它们在支持可靠传输、结构化交互和意义级协调方面的表现。我们的分析显示，当前的协议设计存在明显的不平衡。大多数协议对传输、流式处理、模式（schema）定义和生命周期管理提供了日益成熟的支持，但在澄清、上下文对齐和验证方面提供的协议级机制有限。因此，语义责任往往被推给提示词（prompts）、封装器（wrappers）或特定于应用的编排逻辑，从而产生了隐性的互操作性和维护成本。为了使这一差距具有可操作性，我们进一步识别了当今协议生态系统中的主要技术债形式，并提炼了在不同部署环境下选择协议的实用指南。最后，我们概述了一个关于互操作、安全且语义稳健的智能体生态系统的研究议程，旨在从消息传递迈向共同理解。
