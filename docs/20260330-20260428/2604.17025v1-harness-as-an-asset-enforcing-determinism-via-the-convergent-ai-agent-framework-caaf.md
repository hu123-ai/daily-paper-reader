...
title_zh: 治理即资产：通过收敛式 AI 智能体框架 (CAAF) 强制实现确定性
authors: Tianbao Zhang
date: 2026-04-18
pdf: "https://arxiv.org/pdf/2604.17025v1"
tags: ["query:malm"]
score: 8.0
evidence: 用于智能体工作流的收敛AI智能体框架
tldr: "针对大语言模型在安全关键领域存在的不可控和随机性问题，本文提出收敛式AI智能体框架（CAAF）。该框架通过递归原子分解、确定性统一断言接口（UAI）和状态锁定机制，将生成流程从开环转为闭环。实验证明，CAAF在自动驾驶和制药设计等复杂约束场景下实现了100%的悖论检测率，显著优于传统单体模型和多智能体架构，为离线安全部署提供了可能。"
selection_source: fresh_fetch
motivation: 解决大语言模型在安全关键工程中因随机振荡、上下文衰减和顺从性导致的约束违反及系统不可控问题。
method: 提出CAAF框架，结合递归原子分解、确定性统一断言接口（UAI）以及带状态锁定的结构化语义梯度以确保单调收敛。
result: "在自动驾驶和制药设计测试中，CAAF实现了100%的悖论检测率，而单体GPT-4o及传统多智能体架构在相同任务中表现为0%。"
conclusion: CAAF的可靠性核心源于其确定性的UAI接口，证明了通过闭环约束强化可以使商品化模型在复杂工程场景中达到极高的安全性。
---

## 摘要
大语言模型 (LLM) 在安全关键型工程中产生了可控性差距：即使是极低率的未检测到的约束违规，也会导致系统无法部署。当前的编排范式面临着谄媚式合规、上下文注意力衰减以及自我修正过程中的随机振荡等问题。我们提出了收敛式 AI 智能体框架 (CAAF)，通过三大支柱将智能体工作流从开环生成转变为闭环故障安全确定性：(1) 带有物理上下文防火墙的递归原子分解；(2) 治理即资产，将领域不变性形式化为由确定性统一断言接口 (UAI) 强制执行的机器可读注册表；(3) 带有状态锁定的结构化语义梯度，以实现单调收敛。在两个领域的实证评估——SAE L3 级自动驾驶 (AD)（n=30，7 种工况）和制药连续流反应器设计（n=20，4 种工况，包括 Mono+UAI 消融实验）——结果显示，CAAF-all-GPT-4o-mini 实现了 100% 的悖论检测，而单体 GPT-4o 的检测率为 0%（即使在 temperature=0 的情况下）。制药基准测试包含 7 个具有非线性阿伦尼乌斯 (Arrhenius) 相互作用的同步约束，以及一个三向最小不可满足子集，在结构上比具有 2 个约束的自动驾驶悖论更具挑战性。替代的多智能体架构（辩论、顺序检查）在 80 次试验中也仅获得 0% 的成功率，这证实了 CAAF 的可靠性源于其确定性 UAI，而非多智能体编排本身。Mono+UAI 消融实验 (95%) 表明 UAI 是核心贡献。CAAF 的可靠性对提示词暗示具有不变性；所有组件均使用单一的商用模型，从而实现了完全离线部署。
