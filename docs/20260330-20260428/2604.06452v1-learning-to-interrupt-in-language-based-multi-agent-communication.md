...
tags: ["query:malm"]
score: 9.0
evidence: 多智能体大语言模型系统的通信框架
tldr: 针对大语言模型多智能体通信中信息冗余和计算成本高的问题，本文提出了一种受人类沟通启发的“可中断通信”框架。研究发现现有模型在中断时往往过度自信，为此开发了名为 HANDRAISER 的学习方法，通过预测未来奖励和成本来确定最佳中断点。实验证明该方法在多种任务中显著降低了通信成本并保持了高性能，具有良好的泛化性。
selection_source: fresh_fetch
motivation: 现有的多智能体通信往往过于冗长且缺乏针对性，导致上下文过载和计算资源浪费。
method: 提出了 HANDRAISER 框架，通过学习预测未来奖励和成本来动态决定监听者何时中断说话者。
result: "在多种多智能体场景下，该方法在保持或提升任务性能的同时，将通信成本降低了 32.2%。"
conclusion: 学习中断机制是优化多智能体协作效率的有效手段，且具有良好的跨智能体和跨任务泛化能力。
---

## 摘要
使用大语言模型（LLM）的多智能体系统在各个领域都展示了令人印象深刻的能力。然而，当前的智能体通信面临着输出冗长的问题，这会导致上下文过载并增加计算成本。尽管现有方法侧重于从说话者端压缩消息，但它们难以适应不同的听者并识别相关信息。人类交流中一种有效的方式是允许听者中断并表达意见或要求澄清。受此启发，我们提出了一种可中断的通信框架，允许正在倾听的智能体中断当前的说话者。通过提示实验，我们发现当前的 LLM 往往过度自信，在接收到足够信息之前就进行中断。因此，我们提出了一种学习方法，根据估计的未来奖励和成本来预测合适的中断点。我们在各种多智能体场景中评估了我们的框架，包括双智能体文本猜词游戏、三智能体会议调度和三智能体辩论。实验结果表明，与基准相比，我们的 HANDRAISER 可以在保持相当或更优任务性能的同时，将通信成本降低 32.2%。这种学习到的中断行为也可以推广到不同的智能体和任务中。
