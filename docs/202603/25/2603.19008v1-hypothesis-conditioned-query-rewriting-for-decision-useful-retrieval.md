...
tldr: 针对RAG在决策任务中侧重主题相关性而缺乏区分性证据的问题，本文提出HCQR框架。该方法通过提取假设并重写为支持、区分和验证三类查询，将检索转向证据导向。在MedQA等数据集上的实验证明，HCQR显著提升了答案选择的准确率，优于多种基准方法。
selection_source: fresh_fetch
motivation: 传统的RAG检索往往只关注主题相关性，难以提供足以区分不同候选选项的关键决策证据。
method: 提出HCQR框架，通过生成工作假设并将其重写为支持、区分和验证三类特定查询，实现更精准的证据检索。
result: 在MedQA和MMLU-Med数据集上，HCQR相比基础RAG分别提升了5.9和3.6个百分点，表现优于重排序等基准。
