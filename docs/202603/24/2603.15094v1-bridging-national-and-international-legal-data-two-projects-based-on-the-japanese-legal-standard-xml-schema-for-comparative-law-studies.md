...
title_zh: 连接国家与国际法律数据：基于日本法律标准 XML 模式的两个比较法研究项目
...
## 摘要
本文通过连接两个基于日本法律标准 (JLS) XML 模式的连续研究项目，提出了一个计算比较法的集成框架。第一个项目通过开发从 JLS 到 Akoma Ntoso (AKN) 标准的转换流水线建立了结构互操作性，使日本法规能够集成到基于 LegalDocML 的国际立法数据库中。在此基础上，第二个项目应用多语言嵌入模型和语义文本相似度技术，以识别不同国家法律体系之间的对应条款。一个结合了多语言嵌入、FAISS 检索和交叉编码器 (Cross-Encoder) 重排序的原型系统生成了候选对应关系，并将其可视化为跨司法管辖区网络，以进行探索性比较分析。
