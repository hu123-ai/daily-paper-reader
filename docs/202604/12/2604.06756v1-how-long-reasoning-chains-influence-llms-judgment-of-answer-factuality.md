...
score: 6.0
evidence: 大语言模型作为裁判以及推理链对判决准确性的影响
tldr: 本研究探讨了推理链对LLM裁判评估答案事实性的影响。通过在事实问答和数学推理基准上的实验，发现弱模型裁判容易被流畅但错误的推理误导，而强模型裁判虽能利用推理信息，但仍会被看似高质量的错误推理链欺骗。研究揭示了推理的流畅度和事实性是影响裁判决策的关键，强调了开发能区分真实推理质量与表面流畅性的鲁棒裁判模型的必要性。
selection_source: fresh_fetch
motivation: 旨在探究将生成器的推理过程展示给LLM裁判是否能提升其判断准确性，以及推理链如何影响裁判的决策行为。
method: 在事实问答和数学推理基准上，系统分析了不同能力的LLM裁判在面对包含推理链的答案时的判断表现及受干扰情况。
result: 弱裁判常因推理链的流畅性而误判错误答案，强裁判虽能利用推理作为证据，但仍会被看似高质量的错误推理所误导。
conclusion: LLM裁判易受推理链表面流畅性的偏见影响，亟需开发能精准区分推理真实质量与表面特征的鲁棒评估模型。
---
