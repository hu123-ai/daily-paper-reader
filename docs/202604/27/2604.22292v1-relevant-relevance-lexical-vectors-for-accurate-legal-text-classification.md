...
tags: ["query:q5"]
score: 7.0
evidence: 法院备案文件的法律文本分类
tldr: "针对法律文档分类中现有方法过度依赖元数据或高算力LLM的问题，本文提出ReLeVAnT框架。该方法通过n-gram处理、对比得分匹配和浅层神经网络，利用文档间的判别性词汇特征进行二分类。ReLeVAnT仅需一次性关键词提取，在LexGLUE数据集上达到了99.3%的准确率，为法律文本的高效、精准分类提供了轻量化解决方案。"
selection_source: fresh_fetch
motivation: 现有的法律文档分类方法往往依赖结构化元数据或高昂的计算资源，难以在非结构化语料库中实现高效处理。
method: 提出ReLeVAnT框架，结合n-gram处理、对比得分匹配和浅层神经网络，通过提取判别性关键词进行分类。
result: "在LexGLUE数据集上，该方法实现了99.3%的准确率和98.7%的F1分数，表现优异。"
conclusion: ReLeVAnT证明了利用词汇特征和浅层模型即可在法律文本分类任务中实现极高的准确性与效率。
---
