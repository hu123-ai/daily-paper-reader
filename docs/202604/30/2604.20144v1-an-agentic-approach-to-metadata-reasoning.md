...
tags: ["query:malm"]
score: 7.0
evidence: 元数据推理的智能体方法和自主数据源选择
tldr: "针对LLM智能体在处理多数据集复杂任务时面临的数据源发现难题，本文提出了一种名为Metadata Reasoner的智能体元数据推理方法。该方法结合表搜索引擎与自主元数据咨询，旨在为特定分析任务筛选出既充分又精简的数据源。实验表明，该方法在KramaBench上F1分数达83.16%，显著优于现有基准，且在含噪声的数据湖环境中表现出极强的鲁棒性。"
selection_source: fresh_fetch
motivation: 随着数据量增加和语义复杂化，如何从海量数据源中精准发现并选择相关数据集成为智能体执行任务的关键瓶颈。
method: 提出Metadata Reasoner，利用表搜索引擎检索候选表，并自主分析多维度元数据以确定候选表是否符合任务需求。
result: "在KramaBench数据集上F1分数提升了32个百分点，并在合成BIRD数据湖中实现了99%的低质量数据规避率。"
conclusion: 该智能体推理方法能有效解决复杂数据环境下的数据源选择问题，显著提升了数据发现的准确性和鲁棒性。
---

## 摘要
随着大语言模型（LLM）驱动的自主智能体不断演进，以执行需要整合多个数据集的复杂多步任务，发现相关数据源的问题已成为关键瓶颈。除了可用数据源海量规模带来的挑战外，数据源选择之所以困难，是因为数据的语义极其微妙，需要考虑数据的多个方面。为了解决这一问题，我们引入了 Metadata Reasoner，这是一种基于智能体的元数据推理方法，旨在为给定的分析任务识别出一组既充分又最小的数据源集合。Metadata Reasoner 利用表格搜索引擎检索候选表，然后自主参考可用元数据的各个方面，以确定候选表是否符合任务要求。我们通过一系列实证研究证明了 Metadata Reasoner 的有效性。在用于数据选择的真实世界 KramaBench 数据集上进行的评估显示，我们的方法实现了 83.16% 的平均 F1 分数，比最先进的基准模型高出 32 个百分点。此外，在基于 BIRD 数据湖新创建的合成基准测试上的评估表明，Metadata Reasoner 对数据湖中可能存在的冗余和低质量表格具有极强的鲁棒性。在这种噪声环境下，它在选择正确数据集方面保持了 85.5% 的平均 F1 分数，并在避免低质量数据方面表现出 99% 的成功率。
