...
tags: ["query:malm"]
score: 6.0
evidence: 基于大语言模型的多视角推理框架
tldr: 针对多模态实体链接（MEL）中现有方法过度关注实例特征而忽视多维度证据的问题，本文提出MSR-MEL框架。该框架利用大语言模型（LLM）进行无监督推理，通过离线合成包括实例、群体、词汇和统计在内的多视角证据，并结合LLM增强的上下文图神经网络聚合邻域信息，最后由LLM在线推理得出排序策略，在多个基准测试上达到了领先的无监督性能。
selection_source: fresh_fetch
motivation: 现有MEL方法主要关注实例中心特征，缺乏对更广泛证据及其相互依赖关系的探索。
method: 提出MSR-MEL框架，通过离线合成多视角证据（含图神经网络聚合的群体证据）并利用LLM进行在线多视角证据推理。
result: 在广泛使用的MEL基准测试上，MSR-MEL一致优于现有的最先进无监督方法。
conclusion: 结合多视角证据合成与LLM推理能力，能有效提升无监督多模态实体链接的准确性。
---

## 摘要
多模态实体链接（MEL）是数据管理中的一项基础任务，旨在将具有多种模态的歧义指称映射到知识库中的多模态实体。然而，大多数现有的 MEL 方法主要侧重于优化以实例为中心的特征和证据，而对更广泛形式的证据及其复杂的相互依赖关系探索不足。受人类专家决策过程依赖多视角判断的启发，在这项工作中，我们提出了 MSR-MEL，这是一个结合大语言模型（LLMs）用于无监督 MEL 的多视角证据综合与推理框架。具体而言，我们采用了一个两阶段框架：(1) 离线多视角证据综合，构建了一套全面的证据集。这包括捕捉指称和实体以实例为中心的多模态信息的实例中心证据、聚合邻域信息的组级证据、基于字符串重叠率的词法证据，以及基于简单汇总统计的统计证据。我们框架的一个核心贡献是组级证据的综合，它通过图结构有效地聚合了关键的邻域信息。我们首先构建了 LLM 增强的上下文图。随后，通过非对称教师-学生图神经网络对不同模态进行联合对齐。(2) 在线多视角证据推理，利用 LLM 作为推理模块的能力，分析多视角证据的相关性和语义，从而推导出一种有效的排序策略，在无监督的情况下实现准确的实体链接。在广泛使用的 MEL 基准测试上进行的实验表明，MSR-MEL 始终优于最先进的无监督方法。本文的源代码可在以下地址获取：https://anonymous.4open.science/r/MSR-MEL-C21E/。
