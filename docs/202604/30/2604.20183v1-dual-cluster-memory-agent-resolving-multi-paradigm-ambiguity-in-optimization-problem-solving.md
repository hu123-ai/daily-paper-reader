...
tags: ["query:malm"]
score: 8.0
evidence: 用于复杂推理和优化问题求解的多智能体框架
tldr: 针对大语言模型在处理优化问题时面临的多范式结构歧义，本文提出了一种无需训练的“双簇记忆智能体”（DCM-Agent）。该方法通过构建建模和代码双簇记忆，并将其提炼为方法论、检查清单和陷阱，为模型提供结构化指导。实验表明，DCM-Agent 在多个基准测试中显著提升了性能，并展现出大模型知识引导小模型的“知识继承”特性。
selection_source: fresh_fetch
motivation: 大语言模型在解决优化问题时，常因单一问题存在多种冲突的建模范式而难以生成有效解。
method: 提出 DCM-Agent，通过构建建模与代码双簇记忆并提炼结构化知识，结合记忆增强推理实现动态路径导航与错误修复。
result: "在七个优化基准测试中，DCM-Agent 实现了 11% 至 21% 的平均性能提升。"
conclusion: 该框架不仅有效解决了优化问题的结构歧义，还通过“知识继承”现象证明了其在不同规模模型间的扩展性与效率。
---
