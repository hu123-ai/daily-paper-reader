...
tldr: "针对大语言模型在合同修订中存在的幻觉安全和缺乏严谨约束的问题，本文提出了RCBSF框架。该框架将修订过程建模为Stackelberg博弈，通过全局指令代理（GPA）设定风险预算，引导修订代理（CRA）和验证代理（LVA）进行迭代优化。理论证明该博弈能收敛至最优均衡，实验显示其在风险解决率（RRR）上达到84.21%，显著优于现有基准并提升了Token效率。"
selection_source: fresh_fetch
motivation: 解决大语言模型在自动化合同修订中因幻觉安全和缺乏行为约束而导致的修订质量不稳定问题。
method: 提出一种基于Stackelberg博弈的双层多智能体框架，由全局指令代理设定风险约束，引导修订与验证代理协同优化。
result: "在统一基准测试中实现了84.21%的平均风险解决率，达到了SOTA性能，并显著提高了Token使用效率。"
conclusion: RCBSF通过博弈论建模为法律AI领域的合同修订提供了具备理论保障和高效性能的新范式。
---

## 摘要
尽管大语言模型（LLMs）在法律人工智能（Legal AI）领域得到了广泛应用，但由于幻觉安全性（hallucinated safety）和缺乏严格的行为约束，其在自动化合同修订方面的效用仍受到阻碍。为了解决这些局限性，我们提出了风险约束双层斯塔克尔伯格框架（RCBSF），将修订过程建模为一个非合作斯塔克尔伯格博弈。RCBSF 建立了一个层级化的领导者-跟随者（Leader-Follower）结构，其中全局规范智能体（GPA）对由约束修订智能体（CRA）和局部验证智能体（LVA）组成的跟随者系统施加风险预算，以迭代优化输出。我们提供了理论保证，证明这种双层建模方式能够收敛至一个均衡点，且其效用严格优于无引导的配置。在统一基准测试上的实证验证表明，RCBSF 达到了最先进的性能，超越了迭代基线，平均风险解决率（RRR）达到 84.21%，同时提升了 Token 效率。我们的代码可在 https://github.com/xjiacs/RCBSF 获取。
