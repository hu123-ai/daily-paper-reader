...
tags: ["query:malm"]
score: 8.0
evidence: 使用依赖结构的LLM智能体记忆框架
tldr: "ContextWeaver 是一种针对 LLM 智能体的选择性、依赖结构化内存框架，旨在解决长上下文交互中逻辑结构丢失的问题。它将交互轨迹组织为推理步骤图，通过依赖遍历、路径摘要和执行反馈验证来精准选择上下文。在 SWE-Bench 评测中，该方法在提升 pass@1 性能的同时，显著减少了推理步数和 Token 消耗，证明了逻辑依赖建模在提升智能体效率和稳定性方面的有效性。"
selection_source: fresh_fetch
motivation: 传统的滑动窗口或检索式内存管理方法容易忽略长交互中多步推理所需的因果和逻辑结构。
method: 通过构建推理步骤的依赖图，将推理路径压缩为可重用单元，并结合执行反馈进行轻量级验证。
result: 在 SWE-Bench Verified 和 Lite 基准测试中，ContextWeaver 提升了任务成功率，并降低了推理步数和 Token 使用量。
conclusion: 建模逻辑依赖关系为使用工具的 LLM 智能体提供了一种稳定且可扩展的内存机制。
---
