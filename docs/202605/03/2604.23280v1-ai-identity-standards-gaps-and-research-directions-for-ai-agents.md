...
## 摘要
AI 智能体目前正在跨越组织边界运行真实的交易、工作流和子智能体链，且无需持续的人类监督。这产生了一个当前基础设施无法解决的问题：如何识别、验证一个没有实体、没有持久记忆且没有法律地位的实体，并追究其责任？我们将 AI 身份定义为 AI 智能体被声明的身份与其被观察到的行为之间的持续关系，并受限于两者在任何给定时刻相互对应的置信度。通过对行业趋势、新兴标准和技术文献的结构化调查，我们对智能体身份的全生命周期进行了差距分析，并做出了三项贡献：(1) 在四个维度（基质、持久性、可验证性和法律地位）上对人类身份与 AI 身份进行了结构化比较，表明这种不对称性是根本性的，且在不进行结构性修改的情况下将人类框架扩展到智能体会导致系统性失败；(2) 根据自主智能体的身份需求对当前的技术和监管文件进行了评估，发现没有任何文件能充分解决治理非确定性、跨边界实体的挑战；(3) 识别了五个关键差距（语义意图验证、递归委托问责、智能体身份完整性、治理不透明性与执行，以及运营可持续性），目前尚无技术或监管手段能够解决这些差距。这些差距是结构性的；仅靠更多的工程努力无法弥补。本报告的核心结论是，必须开展关于 AI 身份的基础研究。
