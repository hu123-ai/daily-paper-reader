...
## 摘要
数以百万计的用户转向 AI 模型来满足其信息需求。可以想象，大量的用户查询中包含可能在事实上不准确的假设。先前的研究指出，大语言模型 (LLM) 往往无法挑战此类错误假设，并可能强化用户的错误观点。然而，鉴于最近的进展，特别是模型推理能力的提升，我们重新审视了大推理模型 (LRM) 是否能够对潜在假设进行推理并对用户查询做出适当回应。我们构建了涵盖健康、科学和常识的具有不同程度预设的查询，并利用其评估了多个广泛部署的模型。与非推理模型相比，我们发现推理模型的准确率略高（2-11%），但它们仍然无法挑战大部分（26-42%）的错误预设。此外，推理模型仍然容易受到预设表达强度强弱的影响。
