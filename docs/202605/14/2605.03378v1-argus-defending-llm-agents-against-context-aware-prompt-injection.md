...
tags: ["query:malm"]
score: 8.0
evidence: 防御大语言模型智能体的提示注入攻击
tldr: 本研究针对大语言模型（LLM）智能体面临的上下文感知提示注入攻击，指出当前防御手段在动态复杂环境下的局限性。为此，作者提出了新基准AgentLure以模拟真实攻击场景，并开发了防御机制ARGUS。ARGUS通过构建影响溯源图，追踪不可信上下文对决策的影响，并在执行前验证决策依据的可靠性。实验证明，ARGUS在保持高任务效用的同时，显著降低了攻击成功率，有效增强了智能体的安全性。
selection_source: carryover_cache
motivation: 现有的提示注入防御和基准测试大多忽略了真实场景中智能体决策对动态上下文的依赖，导致无法应对上下文感知的自适应攻击。
method: 提出了ARGUS防御机制，通过构建影响溯源图来追踪不可信信息的传播，并在决策执行前进行溯源感知的审计。
result: "实验显示ARGUS将攻击成功率降低至3.8%，同时保留了87.5%的任务效用，在防御效果和鲁棒性上均优于现有方案。"
conclusion: ARGUS为防御复杂的上下文感知提示注入攻击提供了一种有效的溯源审计方案，显著提升了LLM智能体在现实部署中的安全性。
---

## 摘要
大语言模型（LLM）智能体的兴起，伴随着工具使用、技能和外部知识的增强，引入了新的安全风险。其中，提示注入攻击（攻击者将恶意指令嵌入智能体工作流）已成为主要威胁。然而，现有的基准测试和防御措施存在根本局限性，因为它们假设了上下文无关的设置，即智能体在完全指定的用户指令下工作，且攻击是直接且独立于上下文的。因此，它们未能捕捉到现实世界的部署情况，在这些场景中，智能体的行为通常取决于动态上下文，而不仅仅是用户提示，且攻击者可以针对不同上下文调整其攻击。同样，基于这种狭隘威胁模型构建的现有防御措施忽视了现实世界智能体委托的本质。在本文中，我们提出了 AgentLure，这是一个捕捉上下文相关任务和上下文感知提示注入攻击的基准测试。AgentLure 涵盖了四个智能体领域和跨越不同攻击面的八个攻击向量。我们的评估表明，现有的防御措施在这种设置下往往表现不佳，在智能体系统中应对此类攻击时效果较差。为了解决这一局限性，我们提出了 ARGUS，这是一种为 LLM 智能体强制执行溯源感知决策审计的防御机制。ARGUS 构建了一个影响溯源图，以追踪不可信上下文如何传播到智能体决策中，并在执行前验证决策是否由可信证据支撑。我们的评估显示，ARGUS 将攻击成功率降低至 3.8%，同时保留了 87.5% 的任务效用，显著优于现有防御措施，并对自适应白盒攻击者保持鲁棒性。
