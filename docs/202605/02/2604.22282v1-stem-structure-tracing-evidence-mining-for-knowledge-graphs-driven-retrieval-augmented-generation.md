...
tags: ["query:malm"]
score: 6.0
evidence: 利用知识图谱的检索增强生成
tldr: 针对知识图谱问答中语义不匹配和缺乏全局结构视角的问题，本文提出STEM框架。该框架将多跳推理转化为模式引导的图搜索任务，通过语义到结构的投影将查询分解为关系断言，并利用Triple-GNN生成全局引导子图来辅助证据挖掘。实验证明，STEM在多个多跳推理基准测试中显著提升了检索准确性和证据完整性，达到了SOTA水平。
selection_source: fresh_fetch
motivation: 旨在解决知识图谱结构异构导致的语义不匹配以及现有推理路径检索方法缺乏全局结构视角的问题。
method: 提出STEM框架，通过语义到结构投影构建查询模式图，并利用Triple-GNN生成的全局引导子图指导证据图的检索。
result: 在多个多跳推理基准测试上取得了最先进的性能，显著提高了推理图检索的准确性和完整性。
conclusion: 将多跳推理建模为模式引导的图搜索并结合全局结构信息，能有效提升知识图谱驱动的检索增强生成性能。
---
