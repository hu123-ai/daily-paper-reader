...
tags: ["query:malm"]
score: 6.0
evidence: LLM推理步骤中预测答案的演变过程
tldr: "本研究探讨了大语言模型在思维链（CoT）推理过程中答案确定的时机。通过对Qwen3-4B的实验发现，模型往往在推理早期就已确定最终答案，后续生成的大量推理标记多为事后解释。研究提出了一种基于探测的提前停止策略，在仅损失2%准确率的情况下，平均每条查询可减少500个推理标记，显著降低了推理成本和延迟。"
selection_source: fresh_fetch
motivation: 旨在探究大语言模型在长思维链生成过程中，最终答案究竟是在何时被确定的，以识别是否存在冗余推理。
method: 采用强制答案补全法在推理中间阶段提取预测结果，并研究了包括探测法在内的提前停止策略。
result: "实验显示模型在确定最终答案后平均仍会生成760个冗余标记，且仅有32%的查询在推理过程中会改变预测答案。"
conclusion: 大部分思维链生成是冗余的，通过简单的提前停止策略可以在几乎不影响性能的前提下大幅提升推理效率。
---
