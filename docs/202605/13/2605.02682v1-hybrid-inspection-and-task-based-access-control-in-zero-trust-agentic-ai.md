...
title_zh: 零信任智能体 AI 中的混合检查与基于任务的访问控制
...
tags: ["query:malm"]
score: 8.0
evidence: 分布式智能体协作中的安全和访问控制
tldr: 针对大语言模型代理在调用工具和访问资源时的安全风险，本文提出了一种名为CASA的持续代理语义授权框架。该框架结合了确定性控制与语义检查，通过零信任拦截层监控多轮对话，提取用户意图并验证工具调用的合理性。研究还扩展了ASTRA数据集，并首次在多轮对话场景下验证了基于任务的访问控制（TBAC）的有效性，为零信任代理AI提供了新的安全保障。
selection_source: carryover_cache
motivation: 现有的授权机制缺乏对用户原始意图的可见性，难以防止恶意代理篡改工具调用或过度申请权限。
method: 提出一种混合运行时执行模型，通过零信任拦截层结合确定性控制与两阶段语义检查（任务提取与语义匹配）。
result: 扩展了包含多轮对话的ASTRA数据集，并提供了首个在多轮交互环境下基于任务的访问控制实验结果。
conclusion: CASA框架通过语义对齐确保工具调用符合用户的实际任务需求，有效增强了代理AI系统的安全性。
---

## 摘要
授权大语言模型（LLM）驱动的智能体动态调用工具并访问受保护资源会引入显著的安全风险，且随着智能体参与多轮对话并向分布式协作扩展，这些风险会急剧增加。被攻破或恶意的智能体应用可能会篡改工具调用、伪造结果，或请求超出主体预期任务范围的权限；由于当前的委托授权流程缺乏对原始主体意图的可见性，这些行为在现有流程中可能会被忽视。鉴于此，我们为持续智能体语义授权（CASA）做出了以下贡献。首先，我们提出了一种混合运行时执行模型，该模型结合了由零信任拦截层启用的确定性控制和语义控制。五种确定性控制对消息流实施结构和数据完整性保证，而语义检查层则评估工具调用选择是否与委托给智能体的预期任务一致。其次，不同于以往针对单轮交互的基于任务的访问控制（TBAC）技术，我们将语义层分解为两个阶段：i）任务提取步骤，在拦截层从多轮对话中提取主体的目标；ii）授权服务器上的任务-工具语义匹配步骤，评估所请求的工具是否适用于提取的任务。第三，我们扩展了在之前工作中引入的 ASTRA 数据集，通过生成包含针对给定任务的相关和不相关工具调用的多轮交互新型对话-工具数据集。最后，我们提供了多轮对话下 TBAC 的首个实验结果。
