...
## 摘要
LLM 智能体日益依赖可重用技能（如 `SKILL.md`）来执行复杂任务，但这些产物缺乏可移植性：智能体框架对提示词格式高度敏感，导致同一技能在不同框架下的性能差异巨大。然而，大多数技能最初是以与格式无关的 Markdown 编写的，这导致需要针对每个框架进行昂贵的重写，同时也使安全问题在很大程度上未得到解决，在实践中存在广泛的漏洞。为了解决这一问题，我们提出了 SkCC，这是一种面向 LLM 智能体的编译器，它将经典的编译设计引入到智能体技能开发中。SkCC 以 SkIR 为核心，这是一种强类型中间表示（IR），它将技能语义与特定框架的格式解耦，从而实现了跨智能体框架的可移植部署。在此 IR 之上，静态优化器（Optimizer）强制执行安全约束，在部署前拦截漏洞。SkCC 作为一个四阶段流水线实现，有效地将 $m$ 个技能和 $n$ 个框架之间的适配复杂度从 $O(m \times n)$ 降低到 $O(m + n)$。在 SkillsBench 上的实验表明，SkCC 相比原始版本提供了持续且显著的提升，在 Claude Code 上的通过率从 21.1% 提高到 33.3%，在 Kimi CLI 上从 35.1% 提高到 48.7%。此外，该设计实现了低于 10 毫秒的编译延迟、94.8% 的主动安全触发率，并在不同框架中节省了 10-46% 的运行时 Token。
