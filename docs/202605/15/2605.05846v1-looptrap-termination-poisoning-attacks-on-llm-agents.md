...
motivation: LLM智能体依赖自我评估来决定任务终止，这种自主性容易被恶意注入利用，导致计算资源被无限消耗。
...
## 摘要
现代 LLM 智能体通过在迭代执行循环中运行来解决复杂任务，在循环中，它们反复进行推理、行动并自我评估进度，以确定任务何时完成。在这项工作中，我们表明，虽然这种自主循环促进了自治性，但也引入了一个关键风险：通过在智能体的上下文中注入恶意提示词，攻击者可以扭曲智能体的终止判断，使其认为任务尚未完成，从而导致无限制的计算。为了理解这一威胁，我们将其定义并系统地表征为“终止投毒”（Termination Poisoning），并设计了 10 种具有代表性的攻击策略。通过对 8 个 LLM 智能体和 60 个任务的实证研究，我们证明了不同的 LLM 智能体表现出不同的行为特征，这些特征决定了哪些策略会成功。这些可迁移的模式可以作为针对先前未见的智能体和任务构建有效攻击的原则性指导，从而实现超越手动设计模板的可扩展红队测试。基于这些见解，我们引入了 LoopTrap，这是一个自动化的红队测试框架，它通过利用智能体的行为倾向来合成针对特定目标的恶意提示词。LoopTrap 首先通过轻量级探测，沿着四个漏洞维度构建目标智能体的行为画像。然后，它执行自适应陷阱合成，路由到最有效的策略，并通过自我评分机制选择最佳注入内容。最后，成功的陷阱被抽象为可重用的技能库，而失败的尝试则通过自我反思进行改进，从而确保持续优化。广泛的评估表明，LoopTrap 在 8 个主流智能体上实现了平均 3.57 倍的步数放大，峰值达到 25 倍。
