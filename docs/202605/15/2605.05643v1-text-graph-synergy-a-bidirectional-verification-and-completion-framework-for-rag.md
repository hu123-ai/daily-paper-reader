...
tldr: 针对检索增强生成（RAG）中文本与图谱推理流不对称导致的“信息孤岛”问题，本文提出TGS-RAG框架。该框架通过双向协同机制增强性能：利用图节点全局投票重排文本证据以过滤噪声，并基于文本线索通过内存孤立实体桥接算法恢复被剪枝的图推理路径。实验证明，该方法在多跳推理任务中显著优于现有基准，实现了检索精度与效率的平衡。
selection_source: fresh_fetch
motivation: 传统RAG面临文本证据噪声多和图搜索剪枝导致有效路径丢失的问题，且现有混合方法缺乏深度的文本-图双向协同。
method: 提出TGS-RAG框架，包含图到文本的全局投票重排机制和文本到图的内存孤立实体桥接算法。
result: 在多个多跳推理基准测试中，TGS-RAG显著超越了现有最先进的基线模型。
conclusion: 该研究通过双向验证与补全机制有效解决了文本与图谱间的协同难题，提升了RAG系统的推理能力和检索效率。
---
