...
score: 7.0
evidence: 细粒度专利新颖性预测
tldr: "该研究针对专利新颖性评估中二分类模型缺乏细粒度和易受伪相关影响的问题，提出了FiNE-Patents数据集，包含3,658条具有特征级先验技术引用的专利权利要求。研究将评估范式转变为特征级的联合检索与抽象推理，通过大语言模型（LLM）工作流将权利要求分解为特征并与先验技术比对。实验证明，该方法在段落检索和新颖特征识别上优于基准模型，且对伪相关具有更强的鲁棒性。"
selection_source: fresh_fetch
motivation: 传统的专利新颖性预测仅停留在权利要求层面的二分类，缺乏实际应用所需的细粒度解释性且易受伪相关干扰。
method: 提出了FiNE-Patents数据集，并开发了基于LLM的工作流，通过将权利要求分解为具体特征并结合先验技术段落检索来进行推理预测。
result: 实验结果显示，该方法在段落检索和新颖特征识别任务上表现优异，且相比传统分类器在处理伪相关时更具鲁棒性。
conclusion: 该研究通过引入细粒度数据集和特征级推理框架，为实现透明且精准的专利自动化分析提供了新的方向。
---
