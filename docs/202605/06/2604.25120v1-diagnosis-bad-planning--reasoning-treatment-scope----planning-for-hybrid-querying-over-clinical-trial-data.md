...
title_zh: 诊断：规划与推理不当。治疗：SCOPE——面向临床试验数据混合查询的规划
...
tags: ["query:malm"]
score: 7.0
evidence: 用于复杂领域推理的多LLM规划器框架
tldr: 针对临床试验表格推理中LLM常出现的“推理错误”问题，本文提出SCOPE框架。该框架通过多LLM协作，将复杂任务分解为行选择、结构化规划和执行三个阶段，使推理规则和输出约束显式化。在1500个肿瘤临床试验问题上的实验表明，SCOPE在准确性和效率平衡上优于现有基线模型，为临床数据混合查询提供了有效方案。
selection_source: fresh_fetch
motivation: 现有的LLM在处理临床试验表格时，由于缺乏显式规划，在处理隐含属性推理（如疗法类型、随访状态）时容易出错。
method: 提出SCOPE框架，通过多LLM规划器将任务分解为行选择、显式结构化规划和执行，使推理过程透明化。
result: 在1500个肿瘤临床试验混合推理问题上，SCOPE的准确率显著超过了CoT、TableGPT2和EHRAgent等主流方法。
conclusion: 显式的多LLM规划分解是解决临床试验表格复杂推理问题的有效途径，在准确性与效率之间取得了良好平衡。
---

## 摘要
我们研究临床试验表格推理，其中答案并不直接存储在可见单元格中，而是必须通过归一化、分类、提取或轻量级领域推理，从语义理解中推导出来。受当前大语言模型（LLM）方法在隐式规划假设下常遭遇“推理不当”这一现象的启发，我们专注于模型必须从部分观测的临床试验表格中恢复隐式属性（如治疗类型、添加药物、终点角色或随访状态）的场景。我们提出了 SCOPE（临床试验证据检索的结构化临床混合规划），这是一个基于多 LLM 规划器的框架，将任务分解为行选择、结构化规划和执行。规划器在生成答案之前明确源字段、推理规则和输出约束，从而减少了相对于直接提示的歧义。我们在肿瘤临床试验表格的 1,500 个混合推理问题上评估了 SCOPE，并与 zero-shot、few-shot、chain-of-thought、TableGPT2、Blend-SQL 和 EHRAgent 进行了对比。结果表明，显式的多 LLM 规划提高了基于推理的问题的准确性，同时比更重的智能体基准模型提供了更优的准确率-效率权衡。我们的研究结果将临床试验推理定位为一个独特的表格理解问题，并强调了基于混合规划器的分解是一种有效的解决方案。
