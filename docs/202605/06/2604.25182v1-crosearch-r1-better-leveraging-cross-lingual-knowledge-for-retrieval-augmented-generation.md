...
tags: ["query:malm"]
score: 6.0
evidence: 跨语言知识检索增强生成
tldr: 针对多语言检索增强生成（RAG）中跨语言知识利用不足的问题，本文提出CroSearch-R1框架。该框架基于强化学习（GRPO），通过多轮检索策略动态对齐不同语言的知识，并引入多语言Rollout机制优化推理能力的跨语言迁移。实验证明，该方法能有效利用跨语言互补性，显著提升多语言环境下的RAG性能。
selection_source: fresh_fetch
motivation: 传统的简单拼接多语言知识的方法因语言间差异难以有效提升RAG效果，需要更深层的跨语言知识整合。
method: 提出一种结合搜索增强强化学习（GRPO）的框架，利用多轮检索对齐跨语言证据，并通过多语言Rollout机制优化推理迁移。
result: 实验结果表明，CroSearch-R1能够有效利用跨语言的互补信息，显著增强了多语言集合下的检索增强生成效果。
conclusion: 该研究为多语言RAG提供了一种高效的知识整合与推理优化方案，证明了强化学习在跨语言知识对齐中的潜力。
---
