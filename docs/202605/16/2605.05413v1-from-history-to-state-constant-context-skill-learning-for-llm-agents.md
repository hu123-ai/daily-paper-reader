...
tldr: 本文提出“常数上下文技能学习”框架，通过将程序性知识从提示词转化为模型权重，解决了LLM智能体在长历史下的隐私与成本问题。该框架利用确定性追踪器维护紧凑状态，并结合SFT与在线强化学习优化模型。在多个基准测试中，该方法在显著提升成功率的同时，将每步推理的Token消耗降低了2-7倍，证明了将上下文知识内化为权重的有效性。
selection_source: carryover_cache
motivation: 解决LLM智能体在处理长历史和复杂技能提示时存在的隐私泄露风险、高昂推理成本以及本地模型能力不足的问题。
method: 提出一种将上下文转化为权重的框架，利用确定性追踪器维护紧凑状态，并通过有监督微调和在线强化学习训练任务专用模块。
result: 在ALFWorld、WebShop和SciWorld等任务中，该方法在Qwen和Llama模型上取得了优异性能，且每步推理的Token消耗降低了2至7倍。
conclusion: 该研究证明了将程序性上下文从提示词转移到模型权重中是可行的，能有效提升智能体的效率与可靠性。
---
