...
## 摘要
大语言模型（LLMs）在临床决策支持中的应用面临着重大挑战，即在处理非结构化电子健康记录（EHRs）时存在的“隧道视野”和诊断幻觉问题。为了应对这些挑战，我们提出了一种名为 DxChain 的新型链式临床推理框架，该框架通过模拟由“记忆锚定”、“导航”和“验证”阶段组成的临床医生认知轨迹，将诊断工作流转化为一个迭代过程。DxChain 引入了三项关键的方法论创新，以激发 LLM 的潜力：(i) 一种“先画像后计划”（Profile-Then-Plan）范式，通过建立全景患者基线来缓解冷启动幻觉；(ii) 一种医学思维树（Med-ToT）算法，用于战略性前瞻规划和资源感知导航；(iii) 一种利用“天使-恶魔”对抗性辩论来解决复杂证据冲突的辩证诊断验证程序。在两个真实世界基准测试 MIMIC-IV-Ext Cardiac Disease 和 MIMIC-IV-Ext CDM 上的评估结果表明，DxChain 在诊断准确性和逻辑一致性方面均达到了最先进的性能，为下一代临床 AI 提供了一个模块化且可靠的架构。代码见 https://anonymous.4open.science/r/Dx-Chain。
