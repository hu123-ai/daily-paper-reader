...
tldr: "针对智能眼镜在将视觉数据传输至云端大模型时面临的带宽和能耗限制，本文提出一种意图感知的语义代理通信框架。该框架由云端模型根据上下文推断用户意图，并指导眼镜端自适应地仅传输文本、布局或物体语义等关键信息。实验表明，该方法在保持任务性能的同时，可降低超过50%的带宽需求，并提升了低信噪比环境下的通信鲁棒性。"
selection_source: fresh_fetch
motivation: 智能眼镜持续向云端传输高分辨率原始视觉数据，面临严重的无线带宽限制和设备能耗挑战。
method: 提出一种意图感知框架，利用云端VLM推断意图并指导边缘端眼镜进行针对性的语义预处理与传输。
result: "在多种场景下实现了超过50%的带宽节省，且在低信噪比环境下具有更优雅的性能降级表现。"
conclusion: 使通信资源与用户意图保持一致，是实现高效且稳健的可穿戴AI代理系统的核心路径。
---
