...
title_zh: Prism-Reranker：超越相关性评分——为智能体检索联合生成贡献陈述与证据
authors: Dun Zhang
date: 2026-04-26
...
## 摘要
现代检索流水线越来越多地服务于检索增强生成（RAG）和自主智能体等下游消费者，这些消费者需要的不仅仅是一个标量相关性评分。仅告知调用者“相关程度”的重排序器（reranker）会迫使智能体将整个文档丢入语言模型上下文中，从而在无关段落和模板化内容上浪费 token。我们推出了 Prism-Reranker，这是一系列基于 Qwen3.5 构建的重排序模型，涵盖四种规模（0.8B、2B、4B、9B），其功能超越了标量评分。除了标准的“是/否”相关性判断外，每当判定为“是”时，模型会输出：(i) 一段总结文档如何帮助查询的贡献陈述（contribution statement），以及 (ii) 一段证据段落（evidence passage）：这是一种自包含的重写，在保留所有与查询相关的信号的同时剔除噪声。Prism-Reranker 采用混合目标进行训练，结合了来自强大商业重排序 API 的逐点蒸馏（point-wise distillation）以及针对贡献和证据目标的监督微调。我们从 KaLM-Embedding 的开源聚合中策划训练数据，并利用通过商业搜索 API 为开放域查询检索到的真实网页文档及其 LLM 合成变体进行增强，同时将部分查询重写为关键词风格的重构形式，以使模型适应智能体发出的流量。为了协调开放语料库中不一致的标签并获得清晰的二元监督，我们使用 LLM-as-Judge 集成对数据进行重新标注，该集成聚合了五个前沿 LLM 的投票。在 BEIR 的问答（QA）子集以及针对贡献和证据质量的 LLM 评测中，Prism-Reranker 在所有四种规模上均取得了稳健的结果。我们进一步证明，同样的方案可以扩展现有的基于 LLM 的重排序器，为 Qwen3-Reranker-4B 增强了贡献和证据生成能力，同时将其在 BEIR-QA 上的平均 NDCG@10 相比基础模型提升了 +1.54。模型权重、训练方案和评估套件均已发布。
