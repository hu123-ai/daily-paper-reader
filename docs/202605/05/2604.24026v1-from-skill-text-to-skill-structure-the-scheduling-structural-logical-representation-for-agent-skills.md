...
title_zh: 从技能文本到技能结构：针对智能体技能的调度-结构-逻辑表示
...
tldr: 针对当前大模型智能体技能表示过于依赖非结构化文本、难以被机器高效管理和推理的问题，本文提出了一种名为SSL（调度-结构-逻辑）的结构化表示方法。该方法借鉴经典语言知识表示理论，将技能解构为调度信号、执行结构和逻辑动作三个层次。通过LLM归一化器实现，SSL在技能发现和风险评估任务中显著优于纯文本基线，为提升智能体技能的可检查性、重用性和操作性提供了新路径。
...
conclusion: 显式的、基于源数据的结构化表示能显著提升智能体技能的搜索与审查效率，是迈向可操作技能表示的重要一步。
---

## 摘要
LLM 智能体越来越多地依赖可重用技能，即结合了指令、控制流、约束和工具调用的能力包。然而，在大多数当前的智能体系统中，技能仍由文本密集的工件表示，包括 SKILL.md 风格的文档和结构化记录，其机器可用的证据在很大程度上仍嵌入在自然语言描述中。这给以技能为中心的智能体系统带来了挑战：管理技能集合以及利用技能支持智能体，都需要对调用接口、执行结构和具体的副作用进行推理，而这些内容通常纠缠在单一的文本表层中。因此，技能知识的显式表示可能有助于使这些工件更易于机器获取和利用。借鉴 Schank 和 Abelson 在语言知识表示方面的经典著作中的记忆组织包（Memory Organization Packets）、脚本理论（Script Theory）和概念依赖（Conceptual Dependency），我们引入了据我们所知首个针对智能体技能工件的结构化表示，它解耦了技能级的调度信号、场景级的执行结构以及逻辑级的动作和资源使用证据：即调度-结构-逻辑（SSL）表示。我们使用基于 LLM 的归一化器实例化了 SSL，并在两个任务（技能发现和风险评估）的技能语料库上对其进行了评估，其表现显著优于仅限文本的基准线：在技能发现中，SSL 将 MRR 从 0.573 提高到 0.707；在风险评估中，它将宏 F1 从 0.744 提高到 0.787。这些发现表明，显式的、基于源的结构使智能体技能更易于搜索和审查。它们还表明，SSL 最好被理解为向智能体系统更具可检查性、可重用性和操作可行性的技能表示迈出的实际一步，而不是一个成熟的标准或用于管理和使用技能的端到端机制。
