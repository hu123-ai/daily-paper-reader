...
title_zh: Optimas：一种用于性能优化的智能分析驱动生成式人工智能框架
...
tldr: "Optimas是一个基于多智能体工作流的生成式AI框架，旨在解决大语言模型在代码优化中缺乏性能上下文的问题。它通过将性能诊断报告与文献支持的代码转换策略相结合，实现了从瓶颈识别到代码生成、执行及验证的全自动化流程。在3,410次实验中，Optimas生成的代码100%正确，且在超过98.82%的情况下实现了性能提升，在NVIDIA GPU上平均增益达8.02%-79.09%。"
selection_source: carryover_cache
motivation: 现有的性能分析工具仅能识别瓶颈而无法生成代码，且LLM在缺乏性能上下文时难以产生有效的代码优化方案。
method: 提出一种多智能体框架，利用LLM将多份性能报告的诊断结果映射到成熟的代码转换技术，并集成自动化执行与验证流水线。
result: "在多个基准测试和HPC应用中，该框架实现了100%的代码正确率，并在绝大多数实验中显著提升了GPU性能。"
conclusion: Optimas证明了结合性能分析数据与生成式AI可以有效实现端到端的自动化高性能计算代码优化。
---

## 摘要
大语言模型（LLMs）在自动化代码优化方面展现出巨大潜力。然而，在缺乏性能上下文的情况下，它们难以生成正确且有效的代码转换。现有的性能工具可以识别瓶颈，但无法生成可执行的代码更改。因此，性能优化仍然是一项耗时且需要手动完成的工作，通常仅由具备详细架构理解的专家承担。为了弥补这一差距，我们推出了 Optimas，这是一个基于多智能体工作流构建的模块化、全自动、端到端生成式人工智能框架。Optimas 利用 LLMs 将来自多份报告的性能诊断映射到成熟的、有文献支持的代码转换，同时在单一流水线中统一了见解提取、代码生成、执行和验证。在针对 10 个基准测试和两个高性能计算（HPC）微型应用程序的 3,410 次真实实验中，Optimas 生成的代码 100% 正确，并在超过 98.82% 的实验中提升了性能，在 NVIDIA GPU 上实现了 8.02%-79.09% 的平均增益。
