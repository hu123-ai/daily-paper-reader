...
tldr: 针对大语言模型在处理时间信息时推理能力有限且现有方法缺乏通用性与灵活性的问题，本文提出AdapTime。这是一种自适应时间推理方法，通过LLM规划器动态执行重构、重写和审查三种操作。该方法无需外部工具即可无缝集成到现有模型中，显著提升了模型在处理不同复杂度时间问题时的推理效率和准确性。
...
conclusion: AdapTime通过自适应推理策略有效解决了LLM在时间维度上的认知局限，为提升模型通用推理能力提供了新思路。
---
