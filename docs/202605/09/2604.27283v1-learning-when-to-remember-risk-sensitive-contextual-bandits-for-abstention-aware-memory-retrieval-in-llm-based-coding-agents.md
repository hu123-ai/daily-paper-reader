...
tags: ["query:malm"]
score: 8.0
evidence: LLM编程智能体中的检索增强内存检索
tldr: 针对LLM编程代理在检索外部记忆时可能因表面相似性导致错误注入的问题，本文提出了一种风险敏感的上下文多臂老虎机（RSCB-MC）控制器。该方法将记忆检索视为选择性控制问题，通过评估相关性、不确定性和风险等16个特征，决定是否使用记忆、汇总候选或弃权。实验表明，该方法在保持零误报率的同时，显著提升了调试成功率，证明了在编程任务中安全检索比单纯追求相似度更重要。
selection_source: carryover_cache
motivation: 传统的Top-k检索机制容易受到表面相似性的误导，将不兼容的调试经验注入LLM，导致错误的修复决策。
method: 引入RSCB-MC控制器，将记忆使用建模为风险敏感的控制问题，基于16个上下文特征决定最优动作，并采用重惩罚误报的奖励设计。
result: "在实验中实现了62.5%的成功率且保持了0.0%的误报率，同时具备极低的决策延迟。"
conclusion: 对于编程代理而言，记忆检索的关键在于判断检索内容的安全性，而非仅仅寻找最相似的片段。
---
