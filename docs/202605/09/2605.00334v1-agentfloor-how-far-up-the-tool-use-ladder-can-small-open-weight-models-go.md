...
score: 7.0
evidence: 智能体系统与多步协作
tldr: AgentFloor是一个包含30个任务、六级能力阶梯的基准测试，旨在评估小型开源模型在智能体工作流中的表现。研究通过对16个开源模型与前沿模型进行大规模测试，发现中小型开源模型已足以胜任短程、结构化的工具调用任务，但在长程规划和约束跟踪方面仍落后。该研究为智能体系统设计提供了“小模型处理常规、大模型处理规划”的实用原则。
selection_source: fresh_fetch
motivation: 旨在解决智能体工作流中哪些环节必须使用昂贵的前沿模型，而哪些可以由更廉价的小型开源模型替代的实际路由问题。
method: 引入了AgentFloor基准测试，通过涵盖指令遵循、工具调用到长程规划的六级能力阶梯，对16个开源模型和前沿模型进行了超过1.6万次评估。
result: 中小型开源模型在短程结构化任务中表现优异，最强开源模型在整体得分上可媲美前沿模型，但在复杂长程规划任务中仍显不足。
conclusion: 建议智能体系统采用分层设计，由小型开源模型处理大部分常规动作，而将大型前沿模型保留用于需要深度规划和控制的少数关键任务。
---

## 摘要
生产级智能体系统在处理每个用户请求时会进行多次模型调用，其中大部分调用是简短、结构化且常规的。这提出了一个现有的评估方法无法直接回答的实际路由问题：智能体工作流中的哪些部分真正需要大型前沿智能，而哪些部分可以由较小的模型处理？我们推出了 AgentFloor，这是一个包含 30 个任务的确定性基准测试，被组织为六层能力阶梯，涵盖了指令遵循、工具使用、多步协作以及在持续约束下的长程规划。我们在 16,542 次评分运行中评估了 16 个开源权重模型（参数量从 0.27B 到 32B）以及 GPT-5。我们的结果揭示了模型必要性的清晰边界。中小型开源权重模型已经足以胜任主导实际智能体流水线的大部分短程、结构化工具使用工作；总体而言，最强的开源权重模型在我们的基准测试中达到了与 GPT-5 相当的水平，且运行成本更低、速度更快。差距在需要持续协作和跨多步可靠约束跟踪的长程规划任务中表现得最为明显，在这些任务中，前沿模型仍保持优势，尽管双方都未达到极高的可靠性。我们还发现，这种边界不能仅由规模来解释：某些失败可以通过针对性干预得到改善，但其效果是特定于模型的，而非普遍适用的。这些发现为智能体系统提出了一项实用的设计原则：将较小的开源权重模型用于广泛的常规操作，而将大型前沿模型保留给真正需要深度规划和控制的较窄类别的任务。我们发布了该基准测试、测试框架、参数扫描配置以及完整的运行语料库。
