...
date: 2026-04-23
pdf: "https://arxiv.org/pdf/2604.21501v1"
tags: ["query:malm"]
score: 6.0
evidence: 工具增强的智能体框架
tldr: 岩性识别是地球科学的关键任务，但传统方法多为静态映射，缺乏地质推理能力。本文提出GeoMind，一种基于智能体工作流的岩性分类框架。它将分类建模为包含感知、推理和分析的序列推理过程，通过全局规划器协调工具调用，并引入细粒度过程监督以确保逻辑一致性。该方法在四个基准数据集上表现优异，实现了地质合理且透明的决策过程。
selection_source: fresh_fetch
motivation: 传统岩性分类方法缺乏地质先验知识和循证诊断推理，导致预测结果往往脱离地质实际。
method: 提出GeoMind框架，通过感知、推理和分析模块构建智能体工作流，并利用细粒度过程监督优化中间推理步骤。
result: 在四个测井基准数据集上的实验表明，GeoMind在分类性能上一致优于强基准模型。
conclusion: GeoMind通过模拟地质专家的推理过程，为岩性分类提供了一种高精度、可追溯且符合地质约束的新范式。
---
